...
async def _wait_for_flow_run(flow_run_id: str) -> None:
    ...
    The run executes on a worker, so its logs live in Prefect — mirror them
    here so errors surface in THIS terminal instead of only in the Prefect UI.
    """
...
async def _run(
    ...
    if end_to_end:
        # The standard entry point: submits etl-offline (or runs it inline when
        # the optional deployment isn't registered). Waiting stays a CLI concern.
        dispatch = await dispatch_offline_ingest(
        ...
        # Data step only — the always-registered core coordinator. Forward only
        # the selectors the operator passed; with neither present the
    ...
    logger.info("Flow run created: %s (user_id=%s)", flow_run_id, resolved_user_id)
    await _wait_for_flow_run(flow_run_id)
