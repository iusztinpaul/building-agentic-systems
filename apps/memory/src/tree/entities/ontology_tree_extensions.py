...
layers Tree's personal-assistant vocabulary (``task`` / ``topic`` /
``project``) on top of the canonical POLE+O parent ``object`` registered in
...
``object`` parent's ``subtypes`` frozenset) and
