...
from tree.memory.graph.add_entity import _upsert_pending_same_as_edge
...
from tree.memory.graph.preference_supersession import resolve_supersessions
from tree.memory.graph.review.core import review_duplicate
from tree.memory.graph.review.types import ReviewDecision
...
async def _apply_dream_decisions(
    ...
    * ``action == "merged"`` ⇒ upsert the ``SAME_AS`` audit edge (via the
      ...
      :func:`tree.memory.graph.review.core.review_duplicate` with
...
async def _supersession_sweep(
    *,
    ...
    (:func:`tree.memory.graph.preference_supersession.resolve_supersessions`)
