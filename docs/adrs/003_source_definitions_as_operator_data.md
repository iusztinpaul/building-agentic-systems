...
- **Context references:**
  ...
  - `ADR-002` §3 (data orchestrator/worker topology — the consumer of the source set; unchanged here)
...
as `AppConfig.sources` and read in three places — the offline data orchestrator
(`data_etl_orchestrator`), the online URL router's custom-Substack-domain set
...
ingests is a per-source `scheduled: true` flag plus a `scheduled_only` orchestrator param
...
boolean filter. The constraint is that the data orchestrator reads sources **server-side**
...
3. **The offline data orchestrator selects sources dynamically.** The resolved set is
...
4. **The file IS the schedule selector.** The per-source `scheduled` flag, the
   `scheduled_only` orchestrator param/filter, and the `--scheduled-only` CLI flag are
...
flowchart TD
    ...
    subgraph consumers["consumers"]
        ORCH["data_etl_orchestrator<br/>files + inline (concat), else default"]
...
- **+** A single loader is the one place sources are materialised; the online + arxiv
  consumers and the orchestrator share it, so they cannot drift.
