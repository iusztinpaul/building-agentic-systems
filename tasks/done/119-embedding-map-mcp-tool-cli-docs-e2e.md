...
Round-2 re-review after rollup `tasks/done/120`: the tool's UI suffix, file sentence and `graphs://` description say "embedding map" (live rag-mode `fastmcp call` evidence); graph wording byte-identical; 7 / 14 tool counts untouched. Two polish nits (path printed twice on the map CLI; `graphs://` missing-file text says "graph" / names `visualize_memory_graph`) deferred to a follow-up task. Hand off to the PR Reviewer.

### [PR Reviewer] 2026-09-06 23:45 — Review (feature-level, PR #42)

**VERDICT: NO BLOCKERS**

Reviewed `git diff feat/rag-graphrag-modes...2a58545`: 85 files, +12208/−852 (8 commits: grooming `6839dfc`, tasks #114–#119, rollup #120). Blockers: 0; Nits: 12 (appended to the PR #42 description under "PR review nits").

Evidence checked: the UMAP→HDBSCAN→UMAP-2D recipe against the installed libraries (sklearn 1.9.0 `HDBSCAN(copy='warn')` default confirmed, `copy=True` correct; umap-learn 0.5.12 `n_neighbors` clamp and the `n_jobs=1`/`random_state` rule; `-2`/`-3` = sklearn's inf/NaN codes), sampling determinism (`seed + cluster_id`, stable sort), `write_clustering_run` order/idempotence (delete `run_id != this` → upsert on deterministic `_id` → `$set` chunks; re-run converges; `strict=True` zip), `latest_run_id` all-noise fallback, `load_embedding_map` counts (`unclustered = total − points`, validator-bounded), the run id from the subflow's own `FlowRunContext` (Prefect `runtime.flow_run.get_id`), four sequential per-user phases with per-user isolation and parent-level run-logger lines; every store query and the map loader pinned on `user_id` (the chunk stamp is `_id`-keyed on ids read from a user-scoped query — same as `rag/indexing.py`; Nit 6); the MCP tool reads only, `graphs://` traversal guard covers `embedding-map-*` names; legend/tooltip escaped via `esc()`, banner via `textContent`, JSON embed escapes `</`; `NO_CACHE` on the three I/O tasks, `INPUTS − opik_trace_headers` + prompt version + 90 d on `summarise-cluster`, fail-open only at the flow level (a failed attempt is never cached); lazy heavy imports enforced by AST + fresh-interpreter probes; Sigma 3.0.3 pinned bundle emits `resize`/`afterRender` and exposes `graphToViewport`; deleted fan-out index-ordering tests are replaced by `TestOfflineIndexingPhase` ordering tests and a source guard; every old `test_graph_app.py` test survives in `test_viz_app.py`/`test_graph_tools.py`; ADR-007 Decisions 1/5 match shipped code, ADR-002/005/006 status lines amended, four glossary terms present and used verbatim; ruff check + format clean on the branch.

One real edge found (Nit 1, not a Blocker — reachable only with `min_cluster_size` lowered to ≤ 6): a 5–6 chunk corpus crashes inside UMAP's spectral init instead of the friendly skip.

- NO BLOCKERS — **Nits** (also appended to PR description): 1. [Untested] `core.py:79` guard misses `n ≤ n_components + 1`; 2. [Standards] `numpy` undeclared in `pyproject.toml`; 3. [Simplicity] `store.py` imports `core` (numpy) for `NOISE_LABEL`; 4. [Clean code] third `_get_run_logger` copy in `offline.py`; 5. [Documentation discipline] ADR-007 §2 "rank" vs id-indexed palette, §4 cache key also has `cluster_id`; 6. [Standards] chunk-stamp `UpdateOne` not `user_id`-pinned; 7. [Error handling] `BaseException` in the fail-open; 8. [Performance] sync UMAP fit blocks the event loop; 9. [Standards] dispatch-shell warning wording; 10. [UX] map CLI prints the path twice; 11. [Standards] `graphs://` missing-file text says "graph"; 12. [UX] legend rows without hover / stored `summary` unsurfaced.
