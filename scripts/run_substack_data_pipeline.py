...
async def main() -> None:
    feed_urls = app_config.data_pipeline.substack.feeds
    if not feed_urls:
        print("No substack feeds configured in configs/default.yaml")
        sys.exit(1)
        ...
        while True:
            ...
            if run.state and run.state.is_final():
                if run.state.is_completed():
                    print("\nDone. Substack data pipeline completed successfully.")
                else:
                    print(
